...
private[junit5] class JUnitExecutionListener(
  ...
  def getTopOfMethod(className: String, methodName: String) = Some(
    TopOfMethod(className, "public void " + className + "." + methodName + "()")
  )
      ...
      if (testExecutionResult.getStatus == TestExecutionResult.Status.SUCCESSFUL) {
        ...
        report(
          TestSucceeded(
            ...
            None,
            Some(formatter),
            getTopOfMethod(testClass, testName)
          )
        )
  ...
  private def parseTestDescription(displayName: String): (String, String, String) = {
    val matcher =
